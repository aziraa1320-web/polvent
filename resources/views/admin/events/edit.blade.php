...
<div class="create-form-wrapper">
    <div class="create-form-card">
        <div class="create-form-header">
            <div>
                <h3>
                    ...
                    Edit Event: <span style="color:#60a5fa;margin-left:5px;">{{ $event->title }}</span>
                </h3>
                <p>Ubah detail informasi event di bawah ini</p>
            </div>
        ...
        <div class="create-form-body">
            <form method="POST" action="{{ route('admin.events.update', $event) }}" enctype="multipart/form-data" id="form-create-event">
                @csrf
                @method('PUT')
                ...
                <div class="field-group">
                    ...
                    <textarea name="description" id="description" rows="5"
                        class="field-input {{ $errors->has('description') ? 'is-invalid' : '' }}">{{ old('description', $event->description) }}</textarea>
                    <div class="field-hint">Minimal 20 karakter. Deskripsikan event secara lengkap agar mahasiswa tertarik.</div>
                ...
                <div class="field-row">
                    ...
                    <div class="field-group">
                        ...
                        <input type="number" name="quota" id="quota" value="{{ old('quota', $event->quota) }}"
                            ...
                            min="1" max="10000">
                        <div class="field-hint">Saat ini {{ $event->registrations()->count() }} peserta sudah mendaftar</div>
                ...
                <div class="field-group">
                    <label class="field-label">Poster Event <span class="optional">(opsional, kosongi jika tidak ingin diubah)</span></label>
                    <div class="poster-upload-area" id="posterUploadArea" onclick="document.getElementById('poster').click()">
                        ...
                        <div class="poster-upload-text"><strong>Klik untuk upload</strong> poster event baru</div>
                        <div class="poster-upload-hint">Format: JPG, PNG, WebP · Maksimal 2MB</div>
                    </div>
                    <div class="poster-preview" id="posterPreview" style="display: {{ $event->poster ? 'block' : 'none' }}">
                        <img id="posterPreviewImg" src="{{ $event->poster ? Storage::url($event->poster) : '' }}" alt="Preview">
                        <div class="poster-preview-info">
                            <span id="posterFileName">{{ $event->poster ? 'Poster saat ini' : '-' }}</span>
                            <button type="button" class="poster-preview-remove" onclick="removePoster()">✕ Hapus Pilihan</button>
                        </div>
                    </div>
...
<script>
    ...
    function removePoster() {
        ...
        @if($event->poster)
            // If it has existing poster, revert to it visually
            document.getElementById('posterPreviewImg').src = '{{ Storage::url($event->poster) }}';
            fileName.textContent = 'Poster saat ini';
