...
<style>
    ...
    .form-input:focus { border-color: #1e293b; background: #fff; box-shadow: 0 0 0 3px rgba(30,41,59,0.1); }
    .eye-btn {
    ...
    .remember-label input[type="checkbox"] { width: 15px; height: 15px; accent-color: #1e293b; cursor: pointer; }

    .badge {
        display: inline-block; padding: 0.2rem 0.6rem; background: #fee2e2; color: #991b1b;
        border-radius: 4px; font-size: 0.7rem; font-weight: 800; text-transform: uppercase;
        margin-bottom: 0.85rem; border: 1px solid #fca5a5; letter-spacing: 0.05em;
    }

    .btn-submit {
        width: 100%; padding: 0.85rem;
        background: #1e293b; color: white; border: none; border-radius: 0.6rem;
        font-size: 1rem; font-weight: 700; cursor: pointer; letter-spacing: 0.02em;
        transition: background 0.2s, transform 0.15s, box-shadow 0.2s;
        box-shadow: 0 4px 14px rgba(30,41,59,0.3);
    }
    .btn-submit:hover { background: #0f172a; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(30,41,59,0.4); }
    ...
    .recaptcha-box {
        ...
        background: #f9f9f9; border: 1px solid #d1d5db; border-radius: 4px;
        padding: 12px 14px; height: 74px; cursor: pointer; transition: border-color 0.2s;
        margin-bottom: 1.25rem; user-select: none;
    }
    .recaptcha-box:hover { border-color: #9ca3af; }
    .recaptcha-left-area { display: flex; align-items: center; gap: 14px; }
    .rc-checkbox {
        width: 26px; height: 26px; border: 2px solid #bdbdbd; border-radius: 2px;
        background: #fff; display: flex; align-items: center; justify-content: center;
        flex-shrink: 0; transition: border-color 0.2s;
    }
    .rc-checkbox.verified { border-color: #4caf50; }
    .rc-label { font-size: 14px; color: #3c3c3c; font-weight: 400; }
    .recaptcha-right-area { display: flex; flex-direction: column; align-items: center; gap: 2px; text-align: center; }
    .rc-logo-text { font-size: 9px; color: #9e9e9e; font-weight: 600; letter-spacing: 0.04em; }
    .rc-links { font-size: 8px; color: #9e9e9e; }
    .rc-links a { color: #757575; text-decoration: none; }
    .rc-links a:hover { text-decoration: underline; }
    @keyframes spin { to { transform: rotate(360deg); } }
    .rc-spinner { width: 18px; height: 18px; border: 2.5px solid #e5e7eb; border-top-color: #374151; border-radius: 50%; animation: spin 0.75s linear infinite; }
...
<form method="POST" action="{{ route('admin.login') }}" id="login-form">
    ...
    <div class="recaptcha-box" id="rc-box">
        <div class="recaptcha-left-area">
            <div class="rc-checkbox" id="rc-checkbox"></div>
            <span class="rc-label">Saya bukan robot</span>
        </div>
        <div class="recaptcha-right-area">
            <svg width="32" height="32" viewBox="0 0 64 64" fill="none">
                <path d="M32 4L8 14v18c0 15.46 10.24 29.91 24 33.86C45.76 61.91 56 47.46 56 32V14L32 4z" fill="#374151" opacity="0.1"/>
                <path d="M32 4L8 14v18c0 15.46 10.24 29.91 24 33.86C45.76 61.91 56 47.46 56 32V14L32 4z" stroke="#374151" stroke-width="3" fill="none"/>
                <path d="M23 32c0-4.97 4.03-9 9-9s9 4.03 9 9" stroke="#1e293b" stroke-width="3" stroke-linecap="round"/>
                <circle cx="32" cy="32" r="4" fill="#64748b"/>
                <path d="M41 32c0 4.97-4.03 9-9 9s-9-4.03-9-9" stroke="#94a3b8" stroke-width="3" stroke-linecap="round"/>
            </svg>
    ...
    <button type="submit" class="btn-submit">Otorisasi Masuk</button>
</form>
...
(function () {
    ...
    function setSpinner() { rcCheckbox.innerHTML = '<div class="rc-spinner"></div>'; }
    function setCheckmark() {
        rcCheckbox.classList.add('verified');
        rcCheckbox.innerHTML = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#4caf50" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>';
    }
